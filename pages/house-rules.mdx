...
Actions like the Medicine's Treat Wounds, which list a DC that is gated by character proficiency, will be reduced by your level. For example, a level 4 character attempting an expert Treat Wounds action, which requires a DC 20 Medicine check, would instead make a DC 16 check (DC 20 - 4 levels).
...
Item bonus are removed from the game for all items except basic armor sets. The following changes are made to specific spells and items.

- {{{ref "Magic Weapon"}}} gives the enchanted weapon the magical trait and gives a +1 status bonus to damage per weapon die.
- Alchemical bombs and elixirs that have a item
